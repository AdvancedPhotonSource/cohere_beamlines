...
class InstrTab(QWidget):
    ...
    def init(self, tabs, main_window):
        ...
        gen_layout.addRow("delta (deg)", self.delta)
        self.gamma = QLineEdit()
        gen_layout.addRow("gamma (deg)", self.gamma)
        self.detdist = QLineEdit()
        gen_layout.addRow("detdist (m)", self.detdist)
        self.th = QLineEdit()
        gen_layout.addRow("th (deg)", self.th)
        self.chi = QLineEdit()
        gen_layout.addRow("chi (deg)", self.chi)
        self.phi = QLineEdit()
        gen_layout.addRow("phi (deg)", self.phi)
        self.scanmot = QLineEdit()
        gen_layout.addRow("scan motor", self.scanmot)
        self.scanmot_del = QLineEdit()
        gen_layout.addRow("scan motor delta", self.scanmot_del)
