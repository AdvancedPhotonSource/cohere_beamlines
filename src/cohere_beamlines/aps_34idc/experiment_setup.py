...
def get_dark(detcorrectionsdir, spec_timestamp):
    files = glob.glob('*darkfield.tif', root_dir=detcorrectionsdir)
    files.sort()
...
def get_white(detcorrectionsdir, spec_timestamp):
    files = glob.glob('*whitefield.tif', root_dir=detcorrectionsdir)
    files.sort()
    filestamps = [file[8:16] for file in files]
    filestamps.append(spec_timestamp)
    filestamps.sort()
...
def setup(working_dir, det_name, specfile):
    experiment_dir = os.path.join(working_dir, 'cohere-defaults_1')
    import cohere_ui
    path = Path(cohere_ui.__file__)
    n = 3  # levels up
    conf_templete_dir = path.parents[n - 1]

    templ_confdir = os.path.join(conf_templete_dir, 'cohere-defaults')
    # conf files
    ...
    path = Path(cohere_beamlines.__file__)
    beamline_dir = os.path.join(path.parents[0], beamline)
    templ_configinstrfile = os.path.join(beamline_dir, 'configuration', 'config_instr')

    detcorrectionsdir = os.path.join(beamline_dir, 'detector_corrections', det_name)
    ...
    confdir = os.path.join(experiment_dir, 'conf')
    ...
    configinstrfile = os.path.join(confdir, "config_instr")
    ...
    with open(templ_configinstrfile, 'r') as file:
        filedata = file.read()
    ...
    with open(configinstrfile, 'w') as file:
        file.write(filedata)
    ...
    with open(templ_configdispfile, 'r') as file:
        filedata = file.read()
    filedata = filedata.replace('ANLDIR', experiment_dir)
...
def main():
    ...
    setup(args.working_dir, det_name, args.specfile)
