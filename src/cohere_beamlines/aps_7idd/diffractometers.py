...
class Diffractometer_7iddrobot(Diffractometer):
    ...
    def __init__(self, params):
        super(Diffractometer_7iddrobot, self).__init__('7iddrobot')
    ...
    def check_params(self, params):
        ...
        if 'radius' not in params:
            print('radius not parsed from spec file and not configured')
            raise KeyError('radius not parsed from spec file and not configured')
    ...
    def get_geometry(self, shape, scan, conf_params, **kwargs):
        ...
        detdist = params[self.detectordist_mne] / 1000.0  # convert to meters
        ...
        if scanmot == 'en':  # seems en scans always have to be treated differently since init is unique
            q2 = np.array(qc.area(params['chi'], params['th'], params['phi'], params['yaw'], params['pitch'], deg=True))
        ...
        Astar = qc.transformSample2Lab(Astar, params['wedge'], params['chi'], params['th'], params['phi']) * 10.0  # convert to inverse nm.
        Bstar = qc.transformSample2Lab(Bstar, params['wedge'], params['chi'], params['th'], params['phi']) * 10.0
        Cstar = qc.transformSample2Lab(Cstar, params['wedge'], params['chi'], params['th'], params['phi']) * 10.0
