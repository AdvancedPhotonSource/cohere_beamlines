...
def parse_spec4roi(specfile, scan, det=None):
    """
...
def create_instr(configs, **kwargs):
    ...
    if scan is not None:
        ...
        if 'detector' in instr_config_params:
            det_name = instr_config_params['detector']
        else:
            det_name = None
        if 'specfile' in instr_config_params:
            ...
            det_params = parse_spec4roi(instr_config_params.get('specfile'), first_scan, det=det_name)
